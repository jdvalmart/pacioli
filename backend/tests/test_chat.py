...
class TestChat:
    ...
    def test_history_starts_empty(self, client: TestClient) -> None:
        response = client.get("/chat?month=9&year=2026")
    ...
    def test_ask_persists_conversation(self, client: TestClient) -> None:
        with patch(
            ...
            response = client.post(
                "/chat", json={"question": "¿En qué gasto más?", "month": 9, "year": 2026}
            )

        assert response.status_code == 200
        assert response.json() == {"answer": "Tu gasto más alto es Vivienda.", "error": None}

        history = client.get("/chat?month=9&year=2026").json()
    ...
    def test_ask_saves_question_when_ai_fails(self, client: TestClient) -> None:
        with patch(
            ...
            response = client.post("/chat", json={"question": "Hola", "month": 9, "year": 2026})
    ...
    def test_clear_history(self, client: TestClient) -> None:
        with patch(
            ...
            client.post("/chat", json={"question": "Hola", "month": 9, "year": 2026})

        response = client.delete("/chat?month=9&year=2026")
        assert response.status_code == 200
        assert client.get("/chat?month=9&year=2026").json() == []
    ...
    def test_history_is_isolated_per_month(self, client: TestClient) -> None:
        with patch(
            ...
            client.post("/chat", json={"question": "Septiembre", "month": 9, "year": 2026})

        assert client.get("/chat?month=10&year=2026").json() == []
        assert len(client.get("/chat?month=9&year=2026").json()) == 2
...
class TestAIConfig:
    ...
    def test_update_config_persists(self, client: TestClient) -> None:
        ...
        response = client.put("/ai/config", json=payload)
        assert response.status_code == 200

...
def _create_category(client: TestClient, name: str = "Test") -> int:
    response = client.post(
        "/categories", json={"name": name, "type": "expense", "color": "#FF0000", "icon": "🧪"}
    )
...
class TestCategories:
    ...
    def test_create_duplicate_category_conflict(self, client: TestClient) -> None:
        ...
        response = client.post("/categories", json={"name": "Duplicated", "type": "expense"})
...
class TestSubcategories:
    ...
    def test_list_subcategories_of_category(self, client: TestClient) -> None:
        servicios = next(c for c in client.get("/categories").json() if c["name"] == "Servicios")
        response = client.get(f"/categories/{servicios['id']}/subcategories")
        assert response.status_code == 200
    ...
    def test_create_and_delete_subcategory(self, client: TestClient) -> None:
        cat_id = _create_category(client)
        response = client.post(
            f"/categories/{cat_id}/subcategories", json={"name": "Comida rápida"}
        )
        assert response.status_code == 201
        sub_id = response.json()["id"]

        delete_response = client.delete(f"/categories/subcategories/{sub_id}")
...
class TestTransactions:
    ...
    def test_create_and_list_transaction(self, client: TestClient) -> None:
        ...
        response = client.get("/transactions?month=1&year=2026")
        assert response.status_code == 200
    ...
    def test_update_transaction(self, client: TestClient) -> None:
        ...
        tx = client.get("/transactions?month=1&year=2026").json()[0]
    ...
    def test_delete_transaction(self, client: TestClient) -> None:
        ...
        response = client.delete(f"/transactions/{trans_id}")
        assert response.status_code == 200
        assert client.get("/transactions?month=1&year=2026").json() == []
...
class TestRecurring:
    ...
    def test_materialize_creates_instances_once(self, client: TestClient) -> None:
        ...
        first = client.post("/transactions/materialize?month=2&year=2026")
        assert first.status_code == 200
        assert first.json() == {"created": 1}

        second = client.post("/transactions/materialize?month=2&year=2026")
        assert second.json() == {"created": 0}

        february = client.get("/transactions?month=2&year=2026").json()
...
class TestBudgets:
    ...
    def test_upsert_and_list_budget(self, client: TestClient) -> None:
        ...
        budgets = client.get("/budgets?month=1&year=2026").json()
    ...
    def test_budget_upsert_overwrites(self, client: TestClient) -> None:
        ...
        client.put("/budgets", json={**payload, "amount": "500.00"})
        client.put("/budgets", json={**payload, "amount": "700.00"})
    ...
    def test_delete_budget(self, client: TestClient) -> None:
        cat_id = _create_category(client)
        client.put(
            "/budgets", json={"category_id": cat_id, "month": 1, "year": 2026, "amount": "500.00"}
        )

        response = client.delete(f"/budgets?category_id={cat_id}&month=1&year=2026")
        assert response.status_code == 200
        assert client.get("/budgets?month=1&year=2026").json() == []
...
class TestReports:
    ...
    def test_monthly_summary(self, client: TestClient) -> None:
        ...
        income_cat = client.post("/categories", json={"name": "Ingreso", "type": "income"}).json()[
            "id"
        ]
        ...
        response = client.get("/reports/summary?month=1&year=2026")
        assert response.status_code == 200
    ...
    def test_yearly_summaries(self, client: TestClient) -> None:
        response = client.get("/reports/monthly?year=2026")
        assert response.status_code == 200
    ...
    def test_category_spending(self, client: TestClient) -> None:
        ...
        response = client.get("/reports/category-spending?month=1&year=2026")
        assert response.status_code == 200
        rows = response.json()
    ...
    def test_budget_vs_actual(self, client: TestClient) -> None:
        cat_id = _create_category(client)
        client.put(
            "/budgets", json={"category_id": cat_id, "month": 1, "year": 2026, "amount": "1000.00"}
        )
        _create_transaction(client, cat_id, amount="250.00")

        response = client.get("/reports/budget-vs-actual?month=1&year=2026")
        assert response.status_code == 200
        rows = response.json()
    ...
    def test_export_csv(self, client: TestClient) -> None:
        ...
        response = client.get("/reports/export?month=1&year=2026")
        assert response.status_code == 200
